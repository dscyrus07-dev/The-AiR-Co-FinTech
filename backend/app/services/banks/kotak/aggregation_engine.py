"""
Airco Insights — Kotak Bank Aggregation Engine
===============================================
Financial analytics and aggregation for Kotak Bank transactions.
"""
        ...
        return {
            ...
            "monthly": [
                {"month": m.month, "credits": m.total_credits,
                 "debits": m.total_debits, "net": m.net_flow,
...
class KotakAggregationEngine:
    ...
    def aggregate(
        ...
        self.logger.info("Aggregating %d Kotak transactions", len(transactions))

        total_credits     = sum(t.get("credit") or 0 for t in transactions)
        total_debits      = sum(t.get("debit")  or 0 for t in transactions)
        debit_categories  = self._aggregate_categories(transactions, True)
        credit_categories = self._aggregate_categories(transactions, False)
